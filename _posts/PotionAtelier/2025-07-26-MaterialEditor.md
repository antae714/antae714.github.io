...
게임 렌더링과 아트작업자가 작업하는 툴간의 셰이더 **코드·후처리·감마 처리**가 달라 아트작업자가 **원하는 색**을 재현하기 어렵습니다.  
그래서 게임 렌더링 환경에서 렌더링에 필요한 데이터들을 수정하는 환경이 필요하다 생각하여 머티리얼 노드에디터를 제작하게 되었습니다.  
...
프로파일링 결과 쉐이더 컴파일이 70퍼센트 텍스처리소스 로딩이 20퍼센트를 먹는 상황이였습니다.
``` mermaid
pie showData
    title 로딩시간 
    "쉐이더 컴파일": 75
...
## **해결**
비동기 컴파일, 컴파일된 쉐이더를 캐싱하여 재활용하게 하여 한번 컴파일된 쉐이더는 다시 컴파일하지 않도록 하였습니다.
...
{% capture paragraph %}
# **텍스처 압축툴**
별도의 패키징, 빌드 작업이 없는 엔진이기때문에 이미지 파일을 텍스처용 파일로 변환, 텍스처압축까지 해주는 툴을 제작하였습니다.
이 툴은 **이미지 파일을 텍스처로 변환**하고, **압축 포맷**을 선택하여 최적화된 텍스처를 생성합니다.
**DirectXTex** 라이브러리를 사용하여 BC1~7까지 다양한 압축 포맷을 지원합니다.
