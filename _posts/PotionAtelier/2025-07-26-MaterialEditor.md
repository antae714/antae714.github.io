...
처음 의도였던 아트 작업자의 원본 색상을 그대로 표현하는 목적에서는 다소 벗어났지만,  
머티리얼 노드 에디터를 제작하여 **NPR·PBR 등 다양한 셰이딩 모델을 지원하는 범용 머티리얼 시스템**을 구축했습니다.  
또한 **CPU → GPU 파라미터 전달 구조**를 설계해 색상·강도 등의 값을 **런타임에서 실시간 조정**할 수 있도록 구현했습니다.  
더 나아가 머티리얼 시스템을 **UI에도 적용할 수 있도록 확장**하여, UI 연출의 표현 범위를 크게 넓혔습니다.
