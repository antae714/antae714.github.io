...
Cook–Torrance 반사모델을 구현하여  
현실적인 재질 표현을 실현했습니다.  

환경광은 **IBL** 을 사용하여  
난반사는 Irradiance Map, 정반사는 Prefiltered Environment Map과 BRDF LUT로 처리해  
자연스럽고 물리적으로 타당한 간접광 효과를 구현했습니다.

IBL을 씬마다 다르게 사용하여
아침, 저녁, 밤 등 다양한 시간대의 분위기를 연출할 수 있었습니다.
![morning]({{ '/assets/MyLittleStorage/morning.png' | relative_url }}){: style="width: 33%;" }
![eveningnight]({{ '/assets/MyLittleStorage/eveningnight.png' | relative_url }}){: style="width: 33%;" }
![night]({{ '/assets/MyLittleStorage/night.png' | relative_url }}){: style="width: 33%;" }
...
더 부드러운 그림자를 위해 **VSM**도 검토했으나 개발 기간과 파이프라인 변경 위험을 고려해  
...
PCF7x7과 BRDF를 사용함에 따라 픽셀당 연산량이 크게 증가했습니다.
...
{% capture paragraph %}
## **후처리 구조**
좋은 퀄리티의 게임을위해 렌더링 결과물을 변형해주는
이미지 후처리 시스템을 구축했습니다.

후처리 컴포넌트에서 코드로 만들어진 리플렉션으로 PostProcessData를 상속받은 클래스를 툴에서 생성하여 후처리 체인을 구성할 수 있도록 했습니다. 
...
또한 캐주얼 타이쿤 게임의 특성에 맞게, **외곽선의 색상과 두께를 아트팀에서 지정할 수 있도록** 구현했습니다.
외곽선의 두께는 **Box Blur**를 사용해 원하는 만큼 확장하여 적용했습니다.

![Edge]({{ '/assets/MyLittleStorage/Edge.png' | relative\_url }}){: style="width: 100%;" }
{% endcapture %}
{% include paragraph.html content=paragraph %}

{% capture paragraph %}
### 블룸
블룸은 블룸 커브를 사용하여 밝은 부분을 추출후,
다운샘플링, 가우시안 블러, 업샘플링을 거쳐
...
{% capture paragraph %}
### 톤 매핑
톤 매핑은 HDR로 렌더링된 결과물을 LDR로 변환하는 과정입니다.
이를위해 다양한 톤 매핑 알고리즘을 제공했습니다.
