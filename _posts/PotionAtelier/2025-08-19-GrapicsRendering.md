...
{::nomarkdown}
<details>
  <summary>절두체 컬링</summary>
  <div markdown="1">

> GPU에 그리기 명령을 보내기전에 실제 렌더링 대상인지 확인하는 **절두체 컬링** 시스템을 제작했습니다.  
해당 시스템을 제작하기위해 std::vector를 사용해 정렬을 수행하였습니다.  
>
>그런데 프로세스의 CPU메모리수치가 틱마다 변하는 경우가 발생하였고 코드리뷰 결과
벡터 컨테이너를 사용할때 지역변수로 선언하여 매 틱마다 벡터가 재할당되는 문제가 발생한것을 확인하였습니다.  
따라서 벡터를 멤버변수로 선언하여 재할당을 방지하여 메모리풀과 같은 효과를 얻을수있었습니다.
...
{::nomarkdown}
<details>
  <summary>머티리얼 정렬</summary>
  <div markdown="1">

> 렌더링 파이프라인에서 **머티리얼 정렬**은 매우 중요한 최적화 기법입니다.  
동일한 머티리얼을 사용하는 오브젝트들을 **연속적으로 렌더링**함으로써,
렌더링 파이프라인 상태 변경을 최소화하고 GPU 파이프라인의 효율을 극대화할 수 있습니다.
>
> 이를 위해 `MeshDrawCommand`를 기준으로 정렬하여,
동일 머티리얼을 사용하는 메쉬들을 묶어 렌더링하는 시스템을 구현했습니다.
>
> 인스턴싱처럼 **드로우콜 자체를 줄이는 방식은 아니지만**,
머티리얼 변경을 최소화하여 **상태 전환 비용**을 줄이는 방향의 최적화 효과를 달성했습니다.
...
거기에, 아트팀의 요청으로 더 부드러운 그림자를 위해 **VSM**도 검토했으나 **개발 기간**과 **파이프라인 변경 위험**을 고려해  
...
'우리게임에는 블룸이 있어야한다'는 **아트팀의 강력한 요청**이 있어 블룸 후처리를 구현하게되었습니다.
...
게임의 다양한 표현을 위해 파티클 시스템이 필요하다 생각하였고.  
팀원들과 파티클시스템이 필요한지 토의후에 파티클 시스템을 제작하게되었습니다.  
...
그다음 점위좌표에서 GS를 이용하여 점을 쿼드로 변환 하였습니다.
