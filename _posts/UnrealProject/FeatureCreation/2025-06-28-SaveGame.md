...
언리얼에서 멀티플레이어 게임은 각 유저마다 고유한 ID를 가지고 있습니다.
이러한 정보는 플랫폼마다 다르게 관리되며, 언리얼에서는 `UniqueId`를 사용하여 유저를 식별합니다.


유저 정보를 로드시 닉네임 혹은 `UniqueId`를 사용하여 로드해야합니다.  
`UniqueId`는 `Login`함수에서 세팅되기때문에,
`OnPostLogin`에서 로드처리를 진행하면 됩니다.

다만 모든 로드할 데이터에 대한 초기화는 로드전에 이루어져야 함으로 `OnPostLogin` 전에 초기화가 이루어지고 `OnPostLogin`에서 로드처리를 진행해야합니다.


### 자잘한 최적화
데이터 로드전 초기화값을 네트워크 리플리케이티드되는것을 방지하기위해
푸쉬모델을 사용하였습니다.
