...
**지속적인 서비스**를	제공하기 위해서는 세이브게임의 구조가 변경될 수 있으며, 이때 버전을 관리하는 것이 중요합니다.
```cpp
// .h
...
* **환경별 UID 관리 방식**
  ...
  * **Steam 환경**에서는 온라인 서브시스템이 제공하는 `UniqueId`를 사용하여 유저를 식별했습니다.
  * **OSS_Null 환경**에서는 `PlayerName`을 UID로 대체하여 관리했습니다.

* **UID 세팅 및 데이터 로드 흐름**

  * `UniqueId`는 `Login` 단계에서 할당되므로, 이후 `PostLogin` 시점에서 유저 데이터를 로드하도록 처리했습니다.
  * `PlayerName`은 `ClientTravel` 시 옵션으로 전달되어 `PreLogin`에서 설정되며, 마찬가지로 `PostLogin`에서 데이터를 로드했습니다.
