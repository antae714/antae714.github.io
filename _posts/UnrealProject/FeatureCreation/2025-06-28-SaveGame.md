...
> 서브오브젝트순서는 언리얼이 보장해주지않기때문에 직렬화, 역직렬화시 순서가 달라지는 문제가 발생하였습니다.  
`FArchive`를 직접 직렬화 하는방식은 순서가 중요하기 때문에 서브오브젝트를 `TMap<FName, UObject*>`로 관리하여 이름으로 직렬화, 역직렬화 하도록 수정하였습니다.  
직렬화 할때마다 불필요한 힙할당이 발생하는 문제도 있었지만, 이문제를 해결하기위해 메모리풀을 제작하는것은 오버엔지니어링이라 생각되어 우선순위를 낮추었습니다.
