...
그리고, 해당 `SerializedData`을 사용하여 메모리 라이터, 리더를 사용하여 직렬화 합니다.
...
서브오브젝트 순서는 언리얼이 보장해주지 않기 때문에 직렬화, 역직렬화 시 순서가 달라지는 문제가 발생하였습니다.  
`FArchive`를 직접 직렬화 하는 방식은 순서가 중요하기 때문에 서브 오브젝트를 `TMap<FName, UObject*>`로 관리하여 이름으로 직렬화, 역직렬화 하도록 수정하였습니다.  
직렬화 할 때마다 불필요한 힙 할당이 발생하는 문제도 있었지만, 이 문제를 해결하기 위해 메모리 풀을 제작하는 것은 오버엔지니어링이라 생각되어 우선순위를 낮추었습니다.


{::nomarkdown}
<details>
  <summary>액터 저장/로드 플로우 차트</summary>
  <div markdown="1">
...
* **환경별 UID 관리 방식**
  * **Steam 환경**에서는 스팀 이름을 `FUniqueNetId`를 `BytesToHex`하여 관리했습니다.
  * **OSS_Null 환경**에서는 방입장시 유저이름을 입력받아 `PlayerName`을 `BytesToHex`하여 관리했습니다.  

이를 통해 **UID 관리 및 데이터 로드 구조**를 구축할 수 있었습니다.
