...
{% capture paragraph %}
## **매칭시스템 제작**
 **온라인 서브시스템**의 **세션 인터페이스**를 이용하여 세션기반으로 방을 **생성, 검색, 조인**하는 매칭 시스템을 제작하였습니다.

 게임의 매칭 기능을 위해 별도의 데디케이티드 서버를 구축하는 것이 가장 이상적이지만, 실제로는 **비용 부담과 유지보수 리소스** 측면에서 현실적인 제약이 존재했습니다.  
 따라서 해당 프로젝트에서는 **스팀 플랫폼의 온라인 서브시스템(OSS)을 활용한 호스팅된 세션 방식의 매칭 시스템**을 구축하는 것이 요구사항에 가장 적합한 선택이었습니다.  
 
 다만, 이러한 방식은 **크로스 플랫폼을 지원하지 않으며**, 스팀 클라이언트가 설치된 환경에서만 동작한다는 한계가 있습니다.  
 크로스 플랫폼 매칭을 지원하려면, 언리얼에서 사실상 표준으로 사용되는 **OSS 인터페이스에 맞춰 독립적인 매칭 서버를 별도로 구축하는 방식**이 보다 적합합니다.
...
유저가 잘못된 비밀번호를 입력시 `ClientTravel`후 `PreLogin`에서 서버에서 비밀번호 처리를한다면 클라이언트 유저는 **불필요한 로딩시간**과 **불친절한 거부메세지**를 경험하게 됩니다.  

따라서 세션에 조인하기전에 현재비밀번호로 **접속기능한지를 판별**하는 기능이 필요햇습니다.  
세션생성시 `ExtraSettings`에 추가 프로퍼티로 비밀번호를 추가하여 비밀번호를 설정할수있게 하였습니다.  
하지만 비밀번호같은 민감한 값은 평문 대신 OpenSSL로 생성한 `SHA-256` 해시를 저장해 노출을 막았습니다.    
클라이언트는 조인 요청 전에 입력한 비밀번호의 해시를 **세션에 저장된 해시와 비교**해 **접속 가능 여부를 확인**하게 하였습니다.  
그렇게 `PreLogin` 단계에서 진짜 비밀번호끼리 검증하여 로딩후 거부당하기전에 메세지박스로 알림을 주게 하였습니다.
